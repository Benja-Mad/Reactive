...
func _ready() -> void:
	player_name.text = GameGlobal.instance.get_current_player().name
	ready_button.pressed.connect(_toggle_ready)
	GameGlobal.instance.players_updated.connect(_handle_players_updated)
	GameGlobal.instance.player_updated.connect(func(_id: int) -> void: _update_ready_button())
	GameGlobal.instance.vote_updated.connect(func(_id: int) -> void: _handle_vote_updated())
	if multiplayer.is_server():
		start_timer.timeout.connect(func() -> void: _start_game.rpc())
	_handle_players_updated()
	role_button.visible = GameGlobal.instance.use_roles
	...
	if GameGlobal.instance.use_roles:
		_fill_role_container()
		var role: Statics.Role = GameGlobal.instance.get_current_player().role
...
func _toggle_ready() -> void:
	GameGlobal.instance.set_current_player_vote(not GameGlobal.instance.get_current_player().vote)
	_update_player()


func _update_player() -> void:
	var player_ready: bool = GameGlobal.instance.get_current_player().vote
...
func _handle_players_updated() -> void:
	for child: Node in player_list.get_children():
		child.queue_free()
	waiting_label.visible = GameGlobal.instance.players.size() == 1
	for player: Statics.PlayerData in GameGlobal.instance.players:
	...
	_update_ready_button()
	if multiplayer.is_server():
		GameGlobal.instance.reset_votes()


func _handle_back_pressed() -> void:
	if multiplayer.is_server():
		LobbyGlobal.instance.go_to_host()
	else:
		LobbyGlobal.instance.go_to_join()
...
func _update_role(role: Statics.Role) -> void:
	GameGlobal.instance.set_current_player_role(role)
	role_button.text = Statics.get_role_name(role)
	role_container.hide()
...
func _start_game() -> void:
	GameGlobal.instance.set_current_player_vote(false)
	get_tree().change_scene_to_packed(GameGlobal.instance.main_scene)


func _can_start_game() -> bool:
	var quantity: bool = GameGlobal.instance.players.size() >= GameGlobal.instance.min_players
	var completion: bool = not GameGlobal.instance.use_roles or not GameGlobal.instance.all_roles or _are_all_roles_selected()
	var uniqueness: bool = not GameGlobal.instance.use_roles or not GameGlobal.instance.unique_roles or _are_all_roles_unique()
	var fullness: bool = not GameGlobal.instance.use_roles or _all_players_selected_role()
