...
func run() -> void:
 ...
 var captions: Array[String] = ["NORMAL: haces estables", "WAVE: ventanas reactivas", "BLACKOUT: apagado", "RECOVERY: recuperacion"]
 ...
 for frame in 240:
  caption.text = captions[frame/60]
  if frame == 60: controller.trigger_wave()
  if frame == 120:
   controller.set_signal_target("wave_intensity",0.0)
   controller.trigger_blackout()
  if frame == 180: controller.recover_power()
